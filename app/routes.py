...
from app.database import db,Users,Posts,Notes,Replies
...
def view_my_posts():
    ...
    return render_template("my_posts.html",posts = posts,form=form)
...
def edit_reply(id):
    ...
    if request.method=="POST" and current_user.id==reply.reply_id:
        reply.content=request.form["content"]
        db.session.commit()
        flash("Reply edited Successfully!")
        return redirect(url_for('view_replies',id=reply.post_for_id))
    return redirect(url_for('view_replies',id=reply.post_for_id))

@app.route('/delete_reply/<int:id>',methods=["GET","POST"])
@login_required
def delete_reply(id):
    reply=Replies.query.get_or_404(id)
    post_id=reply.post_for_id
    if current_user.id==reply.reply_id:
        db.session.delete(reply)
        db.session.commit()
        flash("Reply deleted Successfully!")
        return redirect(url_for('view_replies',id=post_id))
    return redirect(url_for('view_replies',id=post_id))
